...
namespace XNAFinalEngine.UI
{
    ...
    public class TextBox : ClipControl
    {
        ...
        {
            base.InitSkin();
            SkinControlInformation = new SkinControl(UIManager.Skin.Controls["TextBox"]);
            ...
            if (Text != null && font != null)
            {
                ...
                for (int i = 0; i < linesDrawn + 1; i++)
                {
                    ...
                    if (Lines[ii] != "")
                    {
                        if (mode == TextBoxMode.Multiline)
                        {
                            Renderer.DrawString(font, Lines[ii], r.Left - horizontalScrollBar.Value, r.Top + (i * sizey), col);
                        }
                        else
                        {
                ...
                private void DrawSelection(Rectangle rect)
        {
            if (!selection.IsEmpty)
            {
                ...
                for (int i = start; i <= end; i++)
                {
                    ...
                    Renderer.Draw(UIManager.Skin.Images["Control"].Texture.XnaTexture, r, Color.FromNonPremultiplied(160, 160, 160, 128));
                }
            }
        } // DrawSelection
        ...
        protected internal override void Update()
        {
            ...
            if (Focused)
            {
                flashTime += EngineManager.FrameTime;
        ...
        protected override void OnKeyPress(KeyEventArgs e)
        {
            ...
            if (UIManager.UseGuide && Guide.IsVisible) return;
        ...
        {
            if (UIManager.UseGuide && !Guide.IsVisible)
            {
