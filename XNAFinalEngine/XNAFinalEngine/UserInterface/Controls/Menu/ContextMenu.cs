...
namespace XNAFinalEngine.UI
{
    ...
    public class ContextMenu : MenuBase
    {
        ...
        public ContextMenu()
        {
            ...
            UIManager.Input.MouseDown += InputMouseDown;
        } // ContextMenu
        ...
        {
            base.InitSkin();
            SkinControlInformation = new SkinControl(UIManager.Skin.Controls["ContextMenu"]);
        } // InitSkin
        ...
        protected override void DrawControl(Rectangle rect)
        {
            ...
            for (int i = 0; i < Items.Count; i++)
            {
                ...
                if (Items[i].Separated && i > 0)
                {
                    ...
                    Renderer.Draw(UIManager.Skin.Controls["Control"].Layers[0].Image.Texture.XnaTexture, r, l1.Text.Colors.Enabled);
                }
                if (ItemIndex != i)
                {
                    if (Items[i].Enabled)
                ...
                {
                    Renderer.Draw(UIManager.Skin.Images["Shared.ArrowRight"].Texture.XnaTexture, rect.Left + LineWidth() - 4, rect.Top + l1.ContentMargins.Top + (i * vsize) + 8, col);
                }
            }
        } // DrawControl
        ...
        protected internal override void Update()
        {
            ...
            if (timer != 0 && time - timer >= UIManager.MenuDelay && ItemIndex >= 0 && Items[ItemIndex].ChildrenItems.Count > 0 && ChildMenu == null)
            {
        ...
        public virtual void Show(Control sender, int x, int y)
        {
            ...
            if (ControlLeftAbsoluteCoordinate + Width > EngineManager.Width)
            {
                Left = Left - Width;
                if (ParentMenu != null && ParentMenu is ContextMenu)
                {
                ...
                {
                    Left = EngineManager.Width - (Parent != null ? Parent.ControlLeftAbsoluteCoordinate : 0) - Width - 2;
                }
            }
            if (ControlTopAbsoluteCoordinate + Height > EngineManager.Height)
            {
                Top = Top - Height;
                if (ParentMenu != null && ParentMenu is ContextMenu)
                {
