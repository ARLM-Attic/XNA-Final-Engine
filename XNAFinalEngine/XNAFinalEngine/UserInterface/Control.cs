...
namespace XNAFinalEngine.UI
{
    ...
    public class Control : Disposable
    {
        ...
        /// </summary>
        internal static ControlsList controlList = new ControlsList();
        ...
        private RenderToTexture renderTarget;
        ...
        public virtual int MaximumWidth
        {
            get
            {
                ...
                if (max > EngineManager.Width) max = EngineManager.Width;
                return max;
            }
            set
            {
        ...
        public virtual int MaximumHeight
        {
            get
            {
                ...
                if (max > EngineManager.Height) max = EngineManager.Height;
                return max;
            }
            set
            {
        ...
        public virtual bool Focused
        {
            ...
            set
            {
                ...
                {
                    if (UIManager.FocusedControl == this)
        ...
        {
            if (UIManager.Skin.Controls != null)
            {
                SkinControl _skinControl = UIManager.Skin.Controls[Utilities.ControlTypeName(this)];
                if (_skinControl != null)
                    SkinControlInformation = new SkinControl(_skinControl);
                else
                    SkinControlInformation = new SkinControl(UIManager.Skin.Controls["Control"]);
            }
            else
            {
        ...
        protected override void DisposeManagedResources()
        {
            ...
            if (UIManager.OrderList != null)
                UIManager.OrderList.Remove(this);
            ...
            if (toolTip != null && !UIManager.Disposing)
            {
                toolTip.Dispose();
            }
        ...
        {
            if (UIManager.ToolTipsEnabled && toolTip != null && tooltipTimer > 0 && (TimeSpan.FromTicks(DateTime.Now.Ticks).TotalMilliseconds - tooltipTimer) >= UIManager.ToolTipDelay)
            {
                tooltipTimer = 0;
                toolTip.Visible = true;
                UIManager.Add(toolTip);
            }
        } // ToolTipUpdate
        ...
        {
            if (UIManager.ToolTipsEnabled && toolTip != null && tooltipTimer == 0)
            {
        ...
        {
            if (UIManager.ToolTipsEnabled && toolTip != null)
            {
                tooltipTimer = 0;
                toolTip.Visible = false;
                UIManager.Remove(toolTip);
            }
        } // ToolTipOut
        ...
        /// <summary>
        /// Prepare render target and draw control.
        /// </summary>
        internal virtual void PreDrawControl()
        {
            if (visible && invalidated)
            {
                OnDrawTexture(new DrawEventArgs(new Rectangle(0, 0, ControlAndMarginsWidth, ControlAndMarginsHeight), EngineManager.GameTime));
                ...
                if (renderTarget == null || renderTarget.Width < ControlAndMarginsWidth || renderTarget.Height < ControlAndMarginsHeight)
                {
                    ...
                    int w = ControlAndMarginsWidth  + (UIManager.TextureResizeIncrement - (ControlAndMarginsWidth  % UIManager.TextureResizeIncrement));
                    int h = ControlAndMarginsHeight + (UIManager.TextureResizeIncrement - (ControlAndMarginsHeight % UIManager.TextureResizeIncrement));

                    if (h > EngineManager.Height) h = EngineManager.Height;
                    if (w > EngineManager.Width) w = EngineManager.Width;
                    
                    if (width > 0 && height > 0)
                    {
                        renderTarget = new RenderToTexture(new System.Drawing.Size(w, h), false, false);
                    }
                    else 
                        renderTarget = null;
                }
        ...
        private void DrawControls(Rectangle rect, bool firstDetach)
        {
            ...
            {
                Rectangle = rect,
                GameTime = EngineManager.GameTime
            };
            OnDraw(args);
        ...
        private void DrawOutline(bool child)
        {
            if (!OutlineRectangle.IsEmpty)
            {
                ...
                Texture2D t = UIManager.Skin.Controls["Control.Outline"].Layers[0].Image.Texture.XnaTexture;
                ...
                Color c = UIManager.Skin.Controls["Control.Outline"].Layers[0].States.Enabled.Color;
        ...
        protected virtual void DrawControl(Rectangle rect)
        {
            ...
            {
                Renderer.Draw(UIManager.Skin.Images["Control"].Texture.XnaTexture, rect, backgroundColor);
            }
        ...
        public virtual void Add(Control control)
        {
            if (control != null)
            {
                if (!childrenControls.Contains(control))
                {
                    ...
                    UIManager.DeviceSettingsChanged += control.OnDeviceSettingsChanged;
                    UIManager.SkinChanging += control.OnSkinChanging;
                    UIManager.SkinChanged += control.OnSkinChanged;
        ...
        private void MouseOverProcess(MouseEventArgs e)
        {
            ...
            if (Cursor != null && UIManager.Cursor != Cursor) 
        ...
        private void ClickProcess(EventArgs e)
        {
            ...
            if ((doubleClickTimer == 0 || (timer - doubleClickTimer > UIManager.DoubleClickTime)) || !doubleClicks)
            {
                TimeSpan ts = new TimeSpan(DateTime.Now.Ticks);
            ...
            else if (timer - doubleClickTimer <= UIManager.DoubleClickTime && (ex.Button == doubleClickButton && ex.Button != MouseButton.None))
            {
                doubleClickTimer = 0;
        ...
        private void PerformResize(MouseEventArgs e)
        {
            if (resizable && !IsMoving)
            {
                ...
                {
                    ResizePosition(e);
                    UIManager.Cursor = Cursor = ResizeCursor();
                }
        ...
        private Cursor ResizeCursor()
        {
            switch (resizeArea)
            {
                case Alignment.TopCenter:
                    {
                        return ((resizeEdge & Anchors.Top) == Anchors.Top) ? UIManager.Skin.Cursors["Vertical"].Resource : Cursor;
                    }
                case Alignment.BottomCenter:
                    {
                        return ((resizeEdge & Anchors.Bottom) == Anchors.Bottom) ? UIManager.Skin.Cursors["Vertical"].Resource : Cursor;
                    }
                case Alignment.MiddleLeft:
                    {
                        return ((resizeEdge & Anchors.Left) == Anchors.Left) ? UIManager.Skin.Cursors["Horizontal"].Resource : Cursor;
                    }
                case Alignment.MiddleRight:
                    {
                        return ((resizeEdge & Anchors.Right) == Anchors.Right) ? UIManager.Skin.Cursors["Horizontal"].Resource : Cursor;
                    }
                case Alignment.TopLeft:
                    {
                        return ((resizeEdge & Anchors.Left) == Anchors.Left && (resizeEdge & Anchors.Top) == Anchors.Top) ? UIManager.Skin.Cursors["DiagonalLeft"].Resource : Cursor;
                    }
                case Alignment.BottomRight:
                    {
                        return ((resizeEdge & Anchors.Bottom) == Anchors.Bottom && (resizeEdge & Anchors.Right) == Anchors.Right) ? UIManager.Skin.Cursors["DiagonalLeft"].Resource : Cursor;
                    }
                case Alignment.TopRight:
                    {
                        return ((resizeEdge & Anchors.Top) == Anchors.Top && (resizeEdge & Anchors.Right) == Anchors.Right) ? UIManager.Skin.Cursors["DiagonalRight"].Resource : Cursor;
                    }
                case Alignment.BottomLeft:
                    {
                        return ((resizeEdge & Anchors.Bottom) == Anchors.Bottom && (resizeEdge & Anchors.Left) == Anchors.Left) ? UIManager.Skin.Cursors["DiagonalRight"].Resource : Cursor;
                    }
            }
            return UIManager.Skin.Cursors["Default"].Resource;
        } // ResizeCursor
